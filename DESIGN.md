...
> **Disk note**: `--volatile=overlay` keeps the writable overlay on a RAM-backed tmpfs (≈ half of host RAM by default). Toolchain downloads / extraction (`actions/setup-go`, `setup-node`) and the runner's own diagnostic logs / temp scratch write hundreds of MB and easily exhaust this tmpfs with `ENOSPC: no space left on device` even when the host disk is nearly empty. To avoid this, the job workspace (`/opt/actions-runner/_work`), tool cache (`_tool`), diagnostic dir (`_diag`), and temp scratch (`_temp`) are all bind-mounted onto real host disk at `runner.workspace_path` (default `/opt/runner/workspaces`, a per-runner subdirectory). The processor creates and chowns these dirs to the container's `runner` uid (1001) before boot, and removes them when the runner exits.
